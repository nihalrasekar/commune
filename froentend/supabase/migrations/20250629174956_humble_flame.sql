/*
  # Update Community Chat Seed Data
  
  This migration updates the chat seed data to have:
  1. Only one community chat room (General Discussion)
  2. Multiple one-to-one private chat rooms between users
  3. Sample messages for realistic chat experience
*/
...
-- Create only one community chat room
...
-- Function to create realistic chat data with one community and private chats
CREATE OR REPLACE FUNCTION create_realistic_chat_data()
RETURNS void AS $$
DECLARE
  user_records RECORD;
  user_ids UUID[];
  user_names TEXT[];
  user_flats TEXT[];
  community_room_id UUID := 'cr-general-skyline';
...
SELECT create_realistic_chat_data();

-- Clean up the function
DROP FUNCTION create_realistic_chat_data();
